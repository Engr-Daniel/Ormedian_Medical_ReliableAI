...
- Scope: Phase 0 — Researcher Setup only, Week 1 of a 24-week program. Design for future GTs without over-engineering. No curriculum scaffold has been created.
...
- Literature: future `resources/literature/` contains `README.md`, `references.bib`, and `literature-map.csv`. Zotero is optional. Research-paper PDFs must not be committed.
...
- Implementation: root `AGENTS.md` contains approved architecture and pacing decisions; root `memory.md` provides persistent project context. No labs, environments, dependencies, slides, or assessments have been implemented. Initial publication to the authorized GitHub remote is in progress.
...
- Define Phase 0 prerequisites and assessment pass criteria during content design.
- Defer detailed Weeks 2–24 allocation until Phase 0 is finished and Phase 1 planning begins.
- Await the owner's next instruction before creating the curriculum scaffold.
...
- Outcome / remaining work: files prepared for initial commit and push; remote publication verification is pending.
